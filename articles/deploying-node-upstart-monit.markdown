...
You will need to replace `username` with the user you want to run node as, `/where/yourprogram.js` with the location of your application, `/var/run/yourprogram.pid` with your program’s pid file and `/var/log/yourprogram.sys.log` with the location of your log file. You can then save this file to `/etc/init/yourprogram.conf` for later use. (Don’t forget to make it executable!) If you are using an older linux distribution, you may need to save the file to `/etc/event.d/yourprogram`
