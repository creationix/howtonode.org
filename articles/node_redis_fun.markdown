...
* [node][] ( we will use [v0.1.29](http://github.com/ry/node/downloads), older versions may have a different API, as will 0.1.30 ) **
* [Redis][]
* [redis-node-client][] - to connect to Redis from node. Use the [bundled version](http://bitbucket.org/nikhilm/snip) as the original hasn't been updated for v0.1.29.  
* [nerve][] - A micro-framework to handle routing
...
\*\* _( NOTE: v0.1.30 was released on the day the article was finished. But it will require significant changes to the dependencies, since promises are out the window. So if you are just getting started and want to follow the article, stick to the older version )_
