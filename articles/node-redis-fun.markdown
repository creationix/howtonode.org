...
First let’s get nerve started so that we get something which works. nerve accepts a list of routes, based or regular expressions, and calls the associated function. For the first run let’s just display the string 'Hello World'. Add the following to snip.js:
...
Now let’s just get `addSnippet()` to echo the form data:
...
### Store the snippet ###
Instead of echoing back the POST data, let’s now store it in Redis, thus finishing half the implementation of the pastebin. We have to:
